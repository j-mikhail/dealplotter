  ...
  class GrouponCA {
    ...
    public function GetDivisionData($SRID, $URL, $LID, $TZ, $Sub = false, $WebD = false) {
      ...
      $NewDeal = array (
        ...
        "StoreName" => array('', 'class="merchantContact"', '">', '<', false),
        "Website" => array('', 'class="merchantContact"', 'href="', '"', false, true),
        "Description" => array('', 'class="merchantContact"', '<p><p>', '<p class="outset"', false),
        "EndDate" => array('', 'id="currentTimeLeft"', 'value="', '"', false)
      );
      ...
      foreach ($Addresses as &$Adr) {
        if ((!is_null($Adr)) && ($Adr != '')) $Locations[] = array(CleanHTML($Adr), 0, 0);
      }
