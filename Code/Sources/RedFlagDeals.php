<?php
  ...
  /*
    Note: RFD returns a gateway page to cURL, rather than the redirect page shown in browsers. As a result,
    the gateway page must be parsed for sub-deals, which are read subsequently.
  */
  ...
  class RedFlagDeals {
    ...
    public function GetDivisionData($SRID, $URL, $LID, $TZ, $Sub = false, $WebD = false) {
      ...
      if ($Sub) {
        list ($Data, $DURL) = GetWebData('http://dealoftheday.redflagdeals.com'.$URL, null, 'dealoftheday.redflagdeals.com_splashIgnored=0');
      } else {
      ...
      if ($Sub) {
        ...
        $Start = stripos($Data, 'timeLeftDigitValue');
        while ($Start !== false) {
          $New = FindData($Data, "timeLeftDigitValue", '</span>', 20, 0, false, $Start, false);
          if ($New !== false) {
            if (stripos($New, 'h') !== false) {
              $New = FindData($Data, 'href="/deal/'.$URL.'/', '"', 6, 0, false, $Start, false);
              if ($New !== false) $MoreDeals[] = $New;
            }
          }
          $Start = stripos($Data, "timeLeftDigitValue", $Start + 20);
        }
